...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, mean, std) in tqdm(enumerate(vali_loader)):
            ...
            outputs = model(args, batch_x, itr)
...
def test(model, test_data, test_loader, args, device, itr):
    ...
    print('test shape:', preds.shape, trues.shape)  # (69,512,1,1)
    preds = preds.reshape(-1, preds.shape[-2], preds.shape[-1])
    trues = trues.reshape(-1, trues.shape[-2], trues.shape[-1])
    print('test shape:', preds.shape, trues.shape)  #(35328,1,1)
